...
{
	/// <summary>
	/// Interface for a service providing currency data exposed by the JobServe API.
	/// </summary>
	public interface ICurrencyService : IRemoteCollectionService<CurrencyCollection>
	{
		/// <summary>
		/// Returns a task that gets an array of currencies - either all if no ids are passed, or
		...
		/// <returns></returns>
		Task<Currency[]> GetCurrencies(IEnumerable<string> ids = null);
		/// <summary>
		/// Retrieves the currency with the specified ID, or null if not found.  Note that this is 
