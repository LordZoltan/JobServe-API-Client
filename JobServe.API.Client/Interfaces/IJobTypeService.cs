...
{
	/// <summary>
	/// Interface for a service providing JobType data exposed by the JobServe API.
	/// </summary>
	public interface IJobTypeService : IRemoteCollectionService<JobTypeCollection>, IClientService
	{
		/// <summary>
		/// Returns a task that gets an array of job types - either all if no ids are passed, or
		...
		/// <returns></returns>
		Task<JobType[]> GetJobTypes(IEnumerable<string> ids = null);
		/// <summary>
		/// Retrieves the job type with the specified ID, or null if not found.  Note that this is 
		/// asynchronous to allow for situations where the underlying collection is to be downloaded
		/// from the API servers first
		/// </summary>
		/// <param name="id"></param>
		/// <returns></returns>
		Task<JobType> GetJobType(string id);
	}
}
