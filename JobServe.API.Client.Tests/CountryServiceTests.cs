...
namespace JobServe.API.Tests
{
	...
	{
		ICountryService GetService(IWebRequestManager requestManager = null)
		{
			return new CountryService(new Client(requestManager ?? new WebRequestManager()));
		}
		...
		{
			//ensures that the array when querying by IDs is identical to the country collection
			//that is loaded from the service
			var service = GetService();

			var items = await service.GetCountries();
		...
		{
			var service = GetService();
			//just get a known country
			var item = await service.GetCountry("GBR");
