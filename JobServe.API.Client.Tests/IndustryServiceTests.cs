...
namespace JobServe.API.Tests
{
	...
	{
		IIndustryService GetService(IWebRequestManager requestManager = null)
		{
			return new IndustryService(new Client(requestManager ?? new WebRequestManager()));
		}
		...
		{
			//ensures that the array when querying by IDs is identical to the Industry collection
			//that is loaded from the service
			var service = GetService();

			var items = await service.GetIndustries();
		...
		{
			var service = GetService();
			//just get a known Industry
			var item = await service.GetIndustry("01");
